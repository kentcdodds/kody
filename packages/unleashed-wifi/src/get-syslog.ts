...
export async function getSyslog(input: { reason?: string } = {}) {
	...
	const syslogRecords = extractElements(result.xml, 'syslog')
	const firstSyslog = syslogRecords[0]
	if (firstSyslog && typeof firstSyslog['rawXml'] === 'string') {
		return { syslog: firstSyslog['rawXml'], xml: result.xml }
	}
