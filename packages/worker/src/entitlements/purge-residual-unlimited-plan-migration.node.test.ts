...
const purgeResidualUnlimitedPlanMigration =
	'0084-purge-residual-unlimited-plan.sql'
...
function applyMigrationsBeforePurge(db: DatabaseSync) {
	db.exec('PRAGMA foreign_keys = ON')
	for (const fileName of readdirSync(migrationsDirectory)
		.filter(
			(file) =>
				file.endsWith('.sql') && file < purgeResidualUnlimitedPlanMigration,
		)
		.sort()) {
...
/**
 * Seed after migrations through 0083: plan is NOT NULL DEFAULT 'free'.
 * Residual stored 'unlimited' (reintroduced after 0083) exercises the
 * deployment-gap purge before code enables a new explicit admin unlimited.
...
function seedPlanRows(db: DatabaseSync) {
	db.exec(`
		...
		VALUES
			...
			('invite-unlimited', 2, 'residual unlimited before 0084', 2, 0, 'unlimited'),
...
test('purge residual unlimited→max updates only exact plan matches and preserves schema under a D1 transaction', () => {
	const db = new DatabaseSync(':memory:')
	applyMigrationsBeforePurge(db)
	seedPlanRows(db)
	...
	expect(beforeUsersSql).toContain(`plan TEXT NOT NULL DEFAULT 'free'`)
	expect(beforeInvitesSql).toContain(`plan TEXT NOT NULL DEFAULT 'free'`)
	expect(beforeUsersSql).not.toContain(`DEFAULT 'unlimited'`)
	expect(beforeInvitesSql).not.toContain(`DEFAULT 'unlimited'`)
	expect(
		db
	...
	applyMigrationLikeD1(db, purgeResidualUnlimitedPlanMigration)
...
test('purge residual unlimited→max fails closed and rolls back when a residual unlimited plan remains', () => {
	const db = new DatabaseSync(':memory:')
	applyMigrationsBeforePurge(db)
	seedPlanRows(db)
