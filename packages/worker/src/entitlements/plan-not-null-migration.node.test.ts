...
const planNotNullMigration = '0081-plan-not-null.sql'
...
function applyMigrationsBeforePlanNotNull(db: DatabaseSync) {
	db.exec('PRAGMA foreign_keys = ON')
	for (const fileName of readdirSync(migrationsDirectory)
		.filter((file) => file.endsWith('.sql') && file < planNotNullMigration)
		.sort()) {
	...
	for (const tableName of inboundUserFkTables) {
		if (tableName === 'invites' && options?.reconcileInvitePlans) {
			const expected = beforeRows.invites.map((row) => ({
				...row,
				plan: row.plan === null ? 'unlimited' : row.plan,
			}))
...
/**
 * Seed after migrations through 0080: residual NULL plans exercise 0081
 * reconciliation before the NOT NULL rebuild.
 */
function seedInboundFkRows(db: DatabaseSync) {
	db.exec(`
		...
		INSERT INTO invites (
			...
			('invite-null', 2, 'residual null before 0081', 1, 0, NULL, NULL),
...
test('plan not-null migration reconciles residual NULLs, rebuilds users/invites, and preserves inbound fks under a D1 transaction', () => {
	...
	expect(invitesTableSql(db)).toMatch(/plan TEXT DEFAULT NULL/)
	expect(invitesTableSql(db)).not.toMatch(/plan TEXT NOT NULL/)
	...
	expect(
		db
			.prepare(
				`SELECT sql FROM sqlite_master
				 WHERE name LIKE '_mig0081_%'
				    OR name = '__migration_assertions'`,
			)
			.all(),
	).toEqual([])

	applyMigrationLikeD1(db, planNotNullMigration)
	...
	expect(
		...
		},
		{
			code: 'invite-null',
			created_by: 2,
			note: 'residual null before 0081',
			max_uses: 1,
			use_count: 0,
			expires_at: null,
			plan: 'unlimited',
...
test('plan not-null migration fails closed and rolls back when reconciliation cannot clear NULL plans', () => {
	const db = new DatabaseSync(':memory:')
	applyMigrationsBeforePlanNotNull(db)
	seedInboundFkRows(db)
	db.prepare(
	...
	db.exec(`
		...
		WHEN OLD.id = 2 AND OLD.plan IS NULL
		BEGIN
			SELECT RAISE(IGNORE);
		END;
	`)

	expect(() => applyMigrationLikeD1(db, planNotNullMigration)).toThrow(
