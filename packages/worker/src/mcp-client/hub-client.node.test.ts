...
const mocks = vi.hoisted(() => ({
	...
	takeConnectionEvents: vi.fn(async () => []),
...
function createEnv() {
	const stub = {
		...
		takeConnectionEvents: mocks.takeConnectionEvents,
...
test('waiting peek dispatches a queued mcp.server.disconnected before ack', async () => {
	...
	mocks.takeConnectionEvents.mockClear()
	...
	mocks.takeConnectionEvents.mockResolvedValueOnce([event])
	...
	expect(mocks.takeConnectionEvents).not.toHaveBeenCalled()
	...
	expect(mocks.takeConnectionEvents).toHaveBeenCalledTimes(1)
