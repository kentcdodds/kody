...
/**
 * Origin HTTP/MCP/OAuth/email/queue handler (ADR 0034), shared by both origin
 * entry modules:
 *
 * - `index.ts` — the dev/test/preview entry. It also exports the Durable
 *   Object and `ctx.exports` WorkerEntrypoint classes that env.test owns
 *   locally and that env.preview's bootstrap deploy briefly owns before the
 *   platform/runtime hand-off (see
 *   docs/contributing/architecture/platform-worker-migration-runbook.md and
 *   the runtime counterpart).
 * - `production-worker.ts` — `env.production.main`. Every env.production
 *   `durable_objects`/`workflows` binding sets `script_name` (kody-platform
 *   or kody-runtime), so origin owns zero Durable Object classes in
 *   production; that entry exports only the `ctx.exports` contracts
 *   production genuinely calls (`KodyFetchGateway`, `JobsHost`).
 *
 * Export only this handler from here so neither entry duplicates the
 * ~600-line request pipeline above. `MCP` is imported (not re-exported)
 * only for its static `.serve()` helper, which builds a fetch handler that
 * forwards to the `MCP_OBJECT` binding — that binding is cross-script in
 * production/preview, so importing the class here never requires either
 * entry to own it locally.
 */
