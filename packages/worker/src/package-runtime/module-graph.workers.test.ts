			...
			modules: {
				...
				'.__kody_packages__/@kentcdodds/ai-chat/.__published_bundle__/2e/index.js':
					[
						...
						"\treturn await capabilities.service_start({ source: input.source ?? 'email' })",
						'}',
					].join('\n'),
				[nestedRuntimeModule]: [
					'const runtime = {}',
...
test('saved package execution exposes packages.invoke when package invoke tools are provided', async () => {
	...
	const bundle = await buildKodyModuleBundle({
		...
		sourceFiles: {
			'package.json': packageJson,
			'src/index.ts': [
				"import { capabilities, packageContext, packages } from 'kody:runtime'",
				'',
				'export default async function main(input = {}) {',
				'\t// Direct capabilities.package_invoke_checked should reject; packages.invokeChecked is the public API.',
				'\tlet directKodyInvokeChecked;',
				'\ttry {',
				'\t\tawait capabilities.package_invoke_checked({',
