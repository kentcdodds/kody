...
**entity: "{type}:{id}"** — detail for one hit (\`capability\` | \`guide\` | \`integration\` | \`mcp-server\` | \`package\` | \`secret\`), or 1–10 refs. The first \`:\` is the type; ids may contain colons (\`capability:mcp:home:set_pin\`). Guide detail is the full markdown when it fits the response budget; oversized guides return a table of contents. Open a heading with \`guide:{id}#{slug}\` (for example \`guide:package_subscriptions#repo.pushed\`) or lines with \`guide:{id}#L165\` / \`guide:{id}#L165-L180\`. Open one package export contract with \`package:{id}#{subpath}\` (for example \`package:home-controls#bond-area-shades\`). Open one package file with \`package:{id}#{path}\` (\`package:{id}#README.md#export-jsdoc\`, \`package:{id}#src/file.ts#L165\`). A fragment that matches an export subpath still opens that export. Capability detail includes an execute snippet. MCP server detail lists discovered tools; after that, call \`kody.mcp["name"].tool_name(args)\`.
