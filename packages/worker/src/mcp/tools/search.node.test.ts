...
test('unscoped search ranks the MCP server instead of dumping every remote tool', async () => {
	...
	const unscopedHomeCapabilities = unscopedHome.matches.filter(
		(match) => match.type === 'capability',
	)
	expect(unscopedHomeCapabilities).toEqual([])
	expect(
	...
	expect(domainListing.matches.map((match) => match.type)).toEqual(
		Array.from({ length: toolNames.length }, () => 'capability'),
	)
})
