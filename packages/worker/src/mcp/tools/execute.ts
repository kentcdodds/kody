...
- \`import { packageStorage } from 'kody:runtime'\` inside saved-package code: \`packageStorage()\` returns the same storage interface bound to the declaring package's own bucket, in the package's own runtime and when the module is statically imported (\`kody:@scope/package/export\`) into an execute call or another package — no \`storageId\` needed. Inline execute code has no package provenance, so \`packageStorage()\` throws there; bind a \`storageId\` and use \`storage\`, or call \`packages.invokeChecked\` to run the owning package in its own context.
