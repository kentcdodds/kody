...
export const packageLifecycleInstructions = `Package lifecycle (use this as the primary mental model):
1. Discover and import before building: use \`search\` to find an existing built-in capability, connected capability, or saved package, inspect the entity detail for its exact call shape, then call it (prefer a static \`kody:@scope/package/export\` import for saved-package exports) rather than reimplementing it. Reach for \`packages.invokeChecked\` only when you need dynamic current-version or target-package runtime semantics.
