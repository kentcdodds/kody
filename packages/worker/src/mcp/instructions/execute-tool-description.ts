...
- \`import { packages } from 'kody:runtime'\` exposes \`packages.check(...)\`, \`packages.invoke(...)\`, and \`packages.invokeChecked(...)\` in saved package runtime contexts and authenticated ad hoc execute calls. Prefer \`packages.invokeChecked({ kodyId: 'github', exportName, params })\` for dynamic current-version package calls: \`kodyId\` is the bare Kody id (for example, \`github\`), not the npm-scoped package name (for example, \`@kentcdodds/github\`). Static cross-package imports such as \`kody:@scope/my-package/export-name\` use the npm-scoped package name and bundle a published snapshot.
