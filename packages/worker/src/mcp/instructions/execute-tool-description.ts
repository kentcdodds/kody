...
- Storage, one rule per context: ad hoc execute code uses \`import { storage } from 'kody:runtime'\` against the \`storageId\` bound to the call; saved-package code always uses \`packageStorage()\` from 'kody:runtime' for the package's own data (package repo checks fail on ambient \`storage\` imports); another package's data goes through that package's export (prefer a static \`kody:@...\` import when a bundled snapshot is enough; use \`packages.invokeChecked\` only when you need the target package's own runtime).
...
- Prefer static package imports for ordinary reuse of trusted (self-authored or adopted) packages: \`import fn from 'kody:@scope/my-package/export-name'\` (npm-scoped package name; bundles a published snapshot into this sandbox with negligible per-call platform overhead after bundling). Static imports are a trust decision — the imported code runs in this sandbox with the whole bundle's grants. \`import { packages } from 'kody:runtime'\` exposes \`packages.check(...)\`, \`packages.invoke(...)\`, and \`packages.invokeChecked(...)\` for the cases that need dynamic semantics or isolation: fresh current-version resolution without republishing the caller, entering the target package's own runtime (\`packageContext\`, \`packageSecrets\`, nested \`packages\` helper), cross-package storage mediation, or running less-trusted (unadopted community fork) code in its own runtime instead of yours. When those apply, prefer \`packages.invokeChecked({ kodyId: 'github', exportName, params })\` over bare \`invoke\`; \`kodyId\` is the bare Kody id (for example, \`github\`), not the npm-scoped name.
