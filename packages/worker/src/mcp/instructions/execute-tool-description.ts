...
- Prefer static package imports for ordinary reuse: \`import fn from 'kody:@scope/my-package/export-name'\` (npm-scoped package name; bundles a published snapshot into this sandbox with negligible per-call platform overhead after bundling). \`import { packages } from 'kody:runtime'\` exposes \`packages.check(...)\`, \`packages.invoke(...)\`, and \`packages.invokeChecked(...)\` for the edge cases that need dynamic semantics: fresh current-version resolution without republishing the caller, entering the target package's own runtime (\`packageContext\`, \`packageSecrets\`, nested \`packages\` helper), or cross-package storage mediation. When those apply, prefer \`packages.invokeChecked({ kodyId: 'github', exportName, params })\` over bare \`invoke\`; \`kodyId\` is the bare Kody id (for example, \`github\`), not the npm-scoped name.
