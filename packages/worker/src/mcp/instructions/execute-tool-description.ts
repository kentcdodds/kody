export const executeToolOverviewDescription = `Run one ephemeral ESM module string with a default export. Bare npm imports are detected and resolved at bundle time using the current registry version; packages must be compatible with the Cloudflare Workers runtime (e.g. \`p-retry\`, \`mailparser\`, \`remark\`), and registry or compatibility failures surface as bundle errors. Prefer existing packages over rewriting helpers. Discover capability names with \`search\`; for one capability’s executable snippet and TypeScript call shape, call \`search\` with \`entity: "{name}:capability"\` or use \`meta_list_capabilities\`.`
...
- User-added MCP server and curated OpenAPI capabilities use namespaced accessors: \`await kody.mcp["server-name"].tool_name(input)\` and \`await kody.openapi["name"].operation_slug(input)\` — never a flat \`kody.kind_instance_capability(...)\` call.
...
- Package reuse, two rules. (1) Target package name known when the code is written → static import: \`import fn from 'kody:@scope/my-package/export-name'\` (npm-scoped package name). Platform (built-in) scopes such as \`@kody\` resolve live for every user without forking — the code runs in the caller's runtime; the caller's own copy of the same name wins. Platform-owned dependencies get no \`packageStorage()\` grant: \`packageStorage()\` inside live platform code fails closed (fork the package if it needs storage in your account). This is the default from execute and from other packages: typed, publish-verified, dependency-graph-visible, zero per-call platform cost. Ad hoc execute bundles per call, so static imports from execute always see the current published version. (2) Target name is data, the call needs the target package's own runtime (\`packageContext\`, \`kody.secretMounts\` package secrets, its own \`packageStorage()\` bucket), or you need exactly-once → \`import { packages } from 'kody:runtime'\` and \`packages.invoke({ kodyId: 'github', exportName, params })\`, the only dynamic call, always contract-checked before invoking. \`kodyId\` is the bare Kody id (for example \`github\`), not the npm-scoped name. Keyless invoke is lean/ephemeral (current published version, run records on failure only); add the optional \`idempotencyKey\` field only for exactly-once needs such as webhook event ids and retried dispatch.
...
- Do not write new values. Load \`coding_guide_get({ guide: "values" })\` to migrate existing \`value_get\` / \`value_list\` rows.`

export const executeToolIdempotencyDescription = `Optional \`idempotencyKey\` (max 256 chars): when set, Kody persists the run eagerly (including successes) with a bounded result snapshot under the run record. A timeout stops observing the sandbox and cooperatively aborts nested package work where possible, but already-started remote side effects may still complete. Do not retry a timed-out side-effecting call without an \`idempotencyKey\`. With the same key, retry to get \`replayed: true\` plus the retained result and \`runId\`, or an \`inProgress: true\` response with \`runId\` while still running — neither re-executes. Poll \`run_get\` with that \`runId\`. Key-less execute stays on-failure-only (successes are not listed in Activity).`
