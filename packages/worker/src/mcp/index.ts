...
- Generated UI code can import \`{ kodyWidget }\` from \`@kody/utils\` and use helpers for follow-up messages, external links, fullscreen requests, \`executeCode(code)\`, secret management, values, forms, OAuth, and secret-aware fetches immediately. Generated UI pages include an import map that resolves \`@kody/utils\` to the hosted runtime bundle and the runtime initializes eagerly on import. \`whenKodyWidgetReady()\` remains available for unusual cases that need an explicit ready promise.
