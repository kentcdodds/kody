...
- The public MCP tools accept optional `conversationId` and `memory_context` fields. Clients should generate and reuse a short `conversationId` across related calls when possible; if omitted, Kody generates one and returns it in `structuredContent.conversationId`.
- Keep `memory_context` short, structured, and task-focused. It is reserved for future memory-aware behavior and is not persisted or used for retrieval yet in this phase.
...
- Optionally pass `memory_context` when you want to attach short, structured task context for future memory-aware behavior.
