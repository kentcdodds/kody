...
- Generated UI code can import \`{ kodyWidget, whenKodyWidgetReady }\` from \`kody:widget-runtime\` and await \`whenKodyWidgetReady()\` before using helpers for follow-up messages, external links, fullscreen requests, \`executeCode(code)\`, secret management, values, forms, OAuth, and secret-aware fetches. Generated UI pages include an import map that resolves \`kody:widget-runtime\` to the hosted runtime bundle. The legacy \`window.kodyWidget\` global still exists for backwards compatibility.
