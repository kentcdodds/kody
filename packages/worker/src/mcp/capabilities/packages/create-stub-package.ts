...
export function buildStubPackageFiles(input: {
	...
	return {
		...
		'README.md': [
			...
			'## What it does',
			'',
			'Write this README for a human: what the package does, who it is for,',
			'prerequisites, setup, and how to tell it is working.',
			'',
			'## Prerequisites',
			'',
			'None yet.',
			...
			'## Setup',
			'',
			'Replace this stub with the real package, then publish.',
			'',
			'## Done when',
			'',
			'A human can follow this README without reading agent runbooks.',
			'',
		].join('\n'),
		'AGENTS.md': [
			`# ${input.name} — agent notes`,
			'',
			'Put imports, smoke tests, and edge cases here. Keep README.md',
			'human-focused; do not make it an agent runbook.',
			'',
			'## Imports',
			'',
			...
			'## Smoke tests',
			'',
			'After publish, call the default export from execute.',
			'',
			'## Edge cases',
			'',
			'None yet.',
