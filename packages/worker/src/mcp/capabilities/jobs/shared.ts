...
export const jobUpsertInputSchema = z
	.object({
		...
		code: z
			...
			.optional()
			.describe(
				'Job source code to publish into the repo-backed job entrypoint. Required when creating a new job.',
			),
		sourceId: z
			.string()
			.min(1)
			.optional()
			.describe(
				'Optional repo-backed source id for this job implementation. Omit on create to let Kody provision one.',
			),
		params: z
