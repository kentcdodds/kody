...
export const secretListCapability = defineDomainCapability(
	capabilityDomainNames.secrets,
	{
		name: 'secret_list',
		description:
			'List available secret references for the signed-in user. Results include metadata such as names, descriptions, allowed hosts, and allowed capabilities. Use `codemode.secret_list({ scope })` inside execute-time code when you want the same metadata from the sandbox. Use `/connect/secret` for user-provided API key, token, and credential entry or rotation. Never ask users to paste secrets, tokens, API keys, passwords, or credentials into chat.',
