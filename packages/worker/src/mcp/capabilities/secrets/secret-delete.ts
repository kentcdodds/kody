...
export const secretDeleteCapability = defineDomainCapability(
	capabilityDomainNames.secrets,
	{
		name: 'secret_delete',
		description:
			'Delete an existing secret reference for the signed-in user. Plaintext values stay hidden. Use `/connect/secret` for user-provided API key, token, and credential entry or rotation. Never ask users to paste secrets, tokens, API keys, passwords, or credentials into chat. Use this to remove execute-time access to a secret.',
