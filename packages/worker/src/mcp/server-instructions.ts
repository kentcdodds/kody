...
execute
- Single ESM module string with a default export. Import runtime APIs from \`kody:runtime\`; they are not ambient sandbox globals. Example: \`import { codemode, refreshAccessToken, createAuthenticatedFetch } from 'kody:runtime'\`. Prefer one \`execute\` when the plan is clear. Full rules for \`fetch\`, placeholders, \`secret_list\` / \`value_get\`, and \`x-kody-secret\`: see the \`execute\` tool description.
