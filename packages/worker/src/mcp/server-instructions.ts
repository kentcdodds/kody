...
- Package secret approvals: saving a user secret or passing an ad hoc \`execute\` smoke test does **not** approve package use. After \`package_save\` / \`package_publish_external_push\`, read \`pending_secret_package_approvals\` from the tool result. Prefer \`bulk_approval_url\` when present (one click for every listed secret); otherwise send each \`approval_url\`. Wait for the user to approve, then verify with \`packages.invokeChecked\` before calling the package complete.
