...
- \`job_schedule_once\`: schedule a one-off repo-backed job for the signed-in user without creating a saved package first.
- Package jobs are schedules owned by a package. For ad hoc one-off work that is not tied to a package, use \`job_schedule_once\`. Package apps are optional UI surfaces declared by the package, not a separate top-level primitive.
...
- Cross-package imports use specifiers such as \`kody:@my-package/export-name\`. Package jobs are owned by packages, ad hoc one-off jobs can be scheduled with \`job_schedule_once\`, and package apps are optional package surfaces.
