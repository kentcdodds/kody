...
- Package secret approvals: self-authored packages get automatic read/use access to user secrets (updating or deleting a user secret from package code still needs an \`allowed_packages\` grant); packages forked from community listings still need explicit approval for read/use too. Saving a user secret or passing an ad hoc \`execute\` smoke test does **not** approve community-fork package use. After \`package_save\` / \`package_publish_external_push\`, read \`pending_secret_package_approvals\` from the tool result (non-null only for community forks). Prefer \`bulk_approval_url\` when present (one click for every listed secret); otherwise send each \`approval_url\`. Wait for the user to approve when required, then verify with \`packages.invokeChecked\` before calling the package complete.
