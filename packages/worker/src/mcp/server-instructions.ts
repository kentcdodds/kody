...
- Official how-to guides from the Kody repo: when creating or materially changing a package, call \`kody_official_guide\` with \`guide: "package_authoring"\`. If the package or workflow depends on a third-party integration, secrets, or OAuth, call \`guide: "integration_bootstrap"\` before saving the dependent package. Then load the relevant setup guide: \`oauth\` for standard third-party OAuth (\`/connect/oauth\`), \`connect_secret\` for secret collection, and \`secret_backed_integration\` for the default non-OAuth secret-backed recipe after bootstrap. If unsure, \`search\` for this capability and load the right guide before implementing.
