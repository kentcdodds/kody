...
- \`scheduler_upsert\`: create a new scheduled codemode job or update an existing one in place. Use \`scheduler_list\` / \`scheduler_get\` to inspect status and next run times, and \`scheduler_run_now\` to trigger the stored job immediately without changing its schedule.
