...
import { createSecretProvidersOptInHandler } from './secret-providers-opt-in.ts'
...
function createPostRequest() {
	return {
		request: new Request('https://kody.example/docs/secret-providers/opt-in', {
			method: 'POST',
		}),
		params: {},
		url: new URL('https://kody.example/docs/secret-providers/opt-in'),
	} as never
}

test('secret providers opt-in sends signed-out users to login and turns the flag on for a signed-in user', async () => {
	const env = createEnv()
	const handler = createSecretProvidersOptInHandler(env)
	...
	expect(loginResponse.headers.get('Location')).toBe(
		'https://kody.example/login?redirectTo=%2Fdocs%2Fsecret-providers',
	)
	...
	expect(optedIn.headers.get('Location')).toBe(
		`https://kody.example${routes.docDetail.href({ slug: 'secret-providers' })}`,
	)
