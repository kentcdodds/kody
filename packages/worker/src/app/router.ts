...
export function createAppRouter(appEnv: AppEnv) {
	...
	router.map(
		routes.accountEmail,
		createAccountEmailHandler(appEnv as unknown as Env),
	)
	router.map(
		routes.accountEmailApi,
		createAccountEmailApiHandler(appEnv as unknown as Env),
	)
	router.map(
		routes.accountEmailApiPost,
		createAccountEmailApiHandler(appEnv as unknown as Env),
	)
	router.map(
