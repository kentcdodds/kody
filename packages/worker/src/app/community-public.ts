...
export function buildForkPrompt(input: { name: string; listingId: string }) {
	return `Use Kody to fork the public package "${input.name}" (listing id: ${input.listingId}). Call communityGet with that listing id first, review the package source for safety and cross-scope imports before publishing anything, update README.md (human setup) and AGENTS.md (imports, smoke tests, edge cases) for my goals, and after adapting it, rate it with communityRate.`
}
...
export function buildInstallAdaptPrompt(input: {
	...
	return `I one-click installed the public package "${input.targetName}" on Kody, but it needs adaptation before it can be published. The fork is an inert source in my account (source_id: ${input.sourceId}). Open it with repoOpenSession, do a read-only safety review of all files, fix the failing publish checks — re-implement or remove any cross-scope kody:@ imports — rewrite README.md (human setup) and AGENTS.md (imports, smoke tests, edge cases) for my goals, then publish with repoPublishSession.`
}

export function buildExistingInstallPrompt(input: { targetName: string }) {
	return `I have the public package "${input.targetName}" installed in my Kody account. Call packageGet for it and read README.md plus AGENTS.md, then adapt it to my needs: keep README.md human-focused, put agent runbooks in AGENTS.md, change behavior if needed, and publish the result.`
}
