...
export function buildConnectOauthCreateHelpersPrompt(integrationName: string) {
	return `Create a thin helpers package for the "${integrationName}" OAuth integration. The saved integration is auth credentials only — not an agent-callable API. The helpers package should be the durable agent-facing surface (exports that call the provider with createAuthenticatedFetch from kody:runtime). Open search({ entity: ["package_authoring:guide", "integration_bootstrap:guide"] }), keep README.md (human setup plus ## Intent) and AGENTS.md (imports, smoke tests), and smoke-test with execute before saving.`
}
