...
test('package invocation API accepts normalized YouTube WebSub token scopes', async () => {
	invocationMockModule.invokePackageExport.mockClear()
	invocationMockModule.invokePackageExport.mockResolvedValue({
		status: 200,
		body: {
	...
	const response = await handlePackageInvocationApiRequest(
		new Request(
			'https://example.com/@my-user/api/package-invocations/youtube-livestream-vod-manager/process-video',
			{
				method: 'POST',
				headers: {
					Authorization: 'Bearer private-token-123',
		...
		await createEnv({
			tokenRow: {
				package_kody_ids_json: JSON.stringify([
					'youtube-livestream-vod-manager',
				]),
				export_names_json: JSON.stringify(['./process-video']),
				sources_json: JSON.stringify(['youtube-websub-proxy']),
			},
		}),
		createContext(),
	)
