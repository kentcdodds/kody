...
The helper implementation still lives in
`runtime-tool-factories.ts` and `run-kody-registry.ts` until that cleanup.
This folder exists so the leftover has one named home to remove.
