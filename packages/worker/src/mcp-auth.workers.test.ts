...
test('authenticated standalone GET SSE is rejected so POST tools stay unblocked', async () => {
	const origin = 'https://example.com'
	const validToken: TokenSummary = {
		id: 'token',
		grantId: 'grant',
	...
	const fetchMcp = () => {
		fetchMcpCalls += 1
		return new Response('should-not-hold-the-session')
	}

	const sseGet = await handleMcpRequestAndDrain({
		request: new Request(`${origin}${mcpResourcePath}`, {
			headers: {
				Authorization: 'Bearer token',
	...
	expect(sseGet.headers.get('Allow')).toBe('POST, DELETE')
	expect(fetchMcpCalls).toBe(0)

	const initialize = await handleMcpRequestAndDrain({
		request: new Request(`${origin}${mcpResourcePath}`, {
			method: 'POST',
			headers: {
				Authorization: 'Bearer token',
	...
	expect(await initialize.text()).toBe('should-not-hold-the-session')
	expect(fetchMcpCalls).toBe(1)
})
