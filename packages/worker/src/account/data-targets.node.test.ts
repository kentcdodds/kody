...
test('final schema drops run projection tables without stale inventory coverage', () => {
	const droppedTables = [
		'workflow_runs',
	...
	const combinedSql = [...deletionStatements, ...exportStatements].join('\n')
	for (const table of droppedTables) {
		expect(combinedSql).not.toMatch(new RegExp(`\\b${table}\\b`, 'u'))
	}
