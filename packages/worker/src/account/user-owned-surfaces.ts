...
export const accountUserOwnedDurableObjectSurfaces: ReadonlyArray<UserOwnedDurableObjectSurface> =
	[
		...
		},
		{
			...
			export: 'include',
			notes:
				'Per-user daily entitlement counters plus expand-phase D1 storage-byte, package-service-state, and deletion-fence/write-lease shadows (one DO per stable userId). Daily counters are authoritative in UserMeter; storage-byte, package-service, and deletion shadows are not (D1 remains sole count/discovery/fence/lease authority for those). Self-prunes stale UTC-day rows inside the DO rather than through a retention cron lane; account deletion purge clears counters, storage shadow, package-service shadow, write-lease shadow, and the inbound delivery ledger while preserving an existing deleting tombstone so a later cutover cannot reopen writes; markAccountDeleting reconciles lease shadows from authoritative D1 via shadowReplaceDeletionState; account export pages counters through the user_meter section via exportCounters (may include additive non-authoritative storageBytesShadow, packageServiceStatesShadow, and sanitized deletionShadow without raw lease token/holder on the first page only).',
		},
		{
