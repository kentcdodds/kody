...
export const accountUserOwnedDurableObjectSurfaces: ReadonlyArray<UserOwnedDurableObjectSurface> =
	[
		...
		},
		{
			...
			export: 'include',
			notes:
				'Per-user daily entitlement counters, storage-byte state, package-service liveness, and deletion-fence/write-lease state (one DO per stable userId). Daily counters and package-service running counts are authoritative in UserMeter; users.d1_storage_bytes is a temporary async mirror (not used for enforcement). For write leases, schema v7 distinguishes authority=do (UserMeter-authoritative when callers supply USER_METER; no D1 row on acquire) from authority=legacy (D1 snapshot). D1 account_write_leases rows are legacy email leases or historical stale pre-retirement leftovers pending audit repair. D1 users.deleting_at remains the permanent point gate and email paths omit env to keep the exact D1 lease path. Self-prunes stale UTC-day rows inside the DO rather than through a retention cron lane; account deletion purge clears counters, storage-byte state, package-service liveness, and write leases while preserving an existing deleting tombstone; markAccountDeleting replaces only legacy rows from the live D1 snapshot and preserves DO-authority rows; account export pages counters through the user_meter section via exportCounters (may include additive storageBytesShadow, packageServiceStatesShadow, and sanitized deletionShadow without raw lease token/holder on the first page only).',
		},
		{
