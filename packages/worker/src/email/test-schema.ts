...
WHERE inbox_id IS NOT NULL AND package_id IS NOT NULL;`,
	...
	user_id TEXT,
	inbox_id TEXT,
	event_type TEXT NOT NULL CHECK (event_type IN ('receive_started', 'received', 'quarantined', 'rejected', 'agent_loop_started', 'agent_loop_completed', 'agent_loop_limit_reached', 'agent_loop_failed', 'send_requested', 'sent', 'failed', 'policy_matched')),
	...
	status TEXT NOT NULL CHECK (status IN ('running', 'completed', 'limit_reached', 'failed')),
	...
	updated_at TEXT NOT NULL
		...
		`CREATE INDEX IF NOT EXISTS idx_email_agent_runs_inbound_message
ON email_agent_runs(inbound_message_id);`,
		`CREATE INDEX IF NOT EXISTS idx_email_agent_runs_thread_created_at
ON email_agent_runs(thread_id, created_at);`,
		`CREATE INDEX IF NOT EXISTS idx_email_agent_runs_user_created_at
ON email_agent_runs(user_id, created_at);`,
	]
