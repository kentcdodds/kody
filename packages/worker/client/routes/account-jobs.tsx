					...
					>
						{detail ? (
							<section mix={css(cardCss)}>
								...
								<MetadataGrid
									items={[
										...
										{
											label: 'Next run',
											value: formatTimestamp(detail.nextRunAt),
										},
										{
											label: 'Last run',
											value: detail.lastRunAt
												? formatTimestamp(detail.lastRunAt)
												: '—',
										},
										{
											label: 'Last duration',
							...
							<div mix={css({ ...cardCss, gap: spacing.sm })}>
								...
								<p mix={css({ margin: 0, color: colors.textMuted })}>
									Pick a scheduled job from the list to inspect runs, toggle the
									kill switch, or manage an ad-hoc schedule.
								</p>
							</div>
						)}
					</AccountManagementLayout>
