...
/**
 * Prompt for a second (or later) account on an integration that already
 * exists. The OAuth app is already saved — this must not recreate it or
 * replace the current connection's tokens.
 */
export function buildAddAccountPrompt(input: {
	label: string
	slug: string
	provider: string
	platform: boolean
	connections: ReadonlyArray<{ name: string; accountLabel: string | null }>
}) {
	const suggestedName = nextSuggestedConnectionName(
		input.slug,
		input.connections.map((connection) => connection.name),
	)
	const existing = input.connections
		.map((connection) => {
			const label = connection.accountLabel?.trim()
			return label && label !== connection.name
				? `${connection.name} (${label})`
				: connection.name
		})
		.join(', ')
	const suggestion = integrationProviderSuggestions.find(
		(provider) => provider.id === input.provider || provider.id === input.slug,
	)
	const guideClause = suggestion?.guideSlug
		? `Load the official Kody setup guide with coding_guide_get({ guide: 'provider_${suggestion.guideSlug}' }) if you need provider-specific steps.`
		: ''
	const connectPath = input.platform
		? `Open /connect/oauth?provider=${suggestedName}&platform=${input.slug} so the new account uses the built-in ${input.label} integration under a new connection name.`
		: `Reuse my existing ${input.label} OAuth app. Connect under the name ${suggestedName} at /connect/oauth?provider=${suggestedName} — do not register a new OAuth app.`

	return [
		`Add another ${input.label} account to my Kody integrations.`,
		`Keep the existing connection${input.connections.length === 1 ? '' : 's'} intact: ${existing}.`,
		'Do not replace, rotate, or overwrite those tokens.',
		connectPath,
		'Ask me what this second account is for and choose the minimal extra access.',
		'Then walk me through authorization and verify the new connection without touching the old one.',
		guideClause,
	]
		.filter(Boolean)
		.join(' ')
