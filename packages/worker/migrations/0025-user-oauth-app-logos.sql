...
-- Assets live in COMMUNITY_ASSETS under content-hashed keys; SVG/ICO
-- sources are rasterized before storage. logo_source distinguishes an
-- explicit upload (beats the catalog SVG) from an auto-favicon (loses
-- to the catalog). favicon_source_host records the registrable domain
-- last used so a changed authorize URL can re-fetch.
