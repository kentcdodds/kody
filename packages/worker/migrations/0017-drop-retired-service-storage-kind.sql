-- Production APP_DB has zero leftover kind='service' inventory (confirmed
-- 2026-08-19). Tighten the CHECK so the retired kind cannot return. Fail
-- closed if a leftover row appears: the INSERT into the next table rejects
-- 'service'.
...
CREATE TABLE user_storage_buckets_next (
	...
	kind TEXT NOT NULL CHECK (
		kind IN ('job', 'package', 'execute', 'repo_session', 'unknown')
	),
