...
function createWidgetHostBridge(options = {}) {
  ...
  function dispatchHostContext(hostContext) {
    ...
    options.onHostContextChanged(
      isRecord(hostContext) ? hostContext : void 0
    );
  }
  ...
  async function requestDisplayMode(mode) {
    ...
    try {
      ...
      return nextMode === "inline" || nextMode === "fullscreen" || nextMode === "pip" ? nextMode : null;
    } catch {
      return null;
    }
  }
...
function coerceValueMetadata(value) {
  ...
  if (typeof record.name !== "string" || typeof record.value !== "string" || typeof record.description !== "string" || scope == null || record.app_id != null && typeof record.app_id !== "string" || typeof record.created_at !== "string" || typeof record.updated_at !== "string" || record.ttl_ms != null && (typeof record.ttl_ms !== "number" || !Number.isFinite(record.ttl_ms) || record.ttl_ms < 0)) {
    return null;
  }
  return {
    name: record.name,
...
function coerceSecretMetadata(value) {
  ...
  const record = value;
  const scope = coerceStorageScope(record.scope);
  if (typeof record.name !== "string" || typeof record.description !== "string" || scope == null || record.app_id != null && typeof record.app_id !== "string" || !Array.isArray(record.allowed_hosts) || !record.allowed_hosts.every((host) => typeof host === "string") || typeof record.created_at !== "string" || typeof record.updated_at !== "string" || record.ttl_ms != null && (typeof record.ttl_ms !== "number" || !Number.isFinite(record.ttl_ms) || record.ttl_ms < 0)) {
    return null;
  }
  return {
    name: record.name,
...
function formDataToObject(formData) {
  ...
  for (const name of new Set(formData.keys())) {
    const all = formData.getAll(name).map((entry) => typeof entry === "string" ? entry : entry.name);
...
function pickLastFormValue(value) {
  if (Array.isArray(value)) {
    ...
    return typeof lastValue === "string" ? lastValue : lastValue == null ? null : String(lastValue);
  }
...
function toStringArray(value) {
  if (Array.isArray(value)) {
    return value.map(
      (entry) => typeof entry === "string" ? entry : String(entry)
    );
  }
...
function setControlValues(form, name, values) {
  ...
  for (const control of controls) {
    if (control instanceof HTMLInputElement) {
      ...
      control.checked = values.includes(controlValue) || control.type === "checkbox" && controlValue === "on" && values.some(
...
function normalizeFetchWithSecretsResult(result) {
  ...
  const headers = isRecord2(result.headers) ? Object.fromEntries(
    Object.entries(result.headers).filter(
      (entry) => typeof entry[1] === "string"
    )
  ) : {};
  const status = typeof result.status === "number" ? result.status : 0;
  const text = typeof result.text === "string" || result.text == null ? result.text : null;
...
function appendLocalMessageLogEntry(text) {
  ...
  timestamp.textContent = formatLocalMessageTimestamp(/* @__PURE__ */ new Date());
...
function coerceSessionEndpoints(value) {
  if (!isRecord2(value)) return null;
  if (typeof value.source !== "string" || typeof value.execute !== "string" || typeof value.secrets !== "string" || typeof value.deleteSecret !== "string") {
    return null;
  }
  return {
    source: value.source,
...
function getBootstrap() {
  ...
  const params = isRecord2(kodyWindow.__kodyGeneratedUiBootstrap.params) ? kodyWindow.__kodyGeneratedUiBootstrap.params : {};
  const appSession = isRecord2(kodyWindow.__kodyGeneratedUiBootstrap.appSession) ? {
    token: typeof kodyWindow.__kodyGeneratedUiBootstrap.appSession.token === "string" ? kodyWindow.__kodyGeneratedUiBootstrap.appSession.token : void 0,
    endpoints: coerceSessionEndpoints(
      kodyWindow.__kodyGeneratedUiBootstrap.appSession.endpoints
    )
  } : null;
  return {
    mode: kodyWindow.__kodyGeneratedUiBootstrap.mode === "entry" || kodyWindow.__kodyGeneratedUiBootstrap.mode === "hosted" || kodyWindow.__kodyGeneratedUiBootstrap.mode === "mcp" ? kodyWindow.__kodyGeneratedUiBootstrap.mode : "entry",
    params,
    appSession: appSession && appSession.endpoints ? {
...
function initializeGeneratedUiRuntime() {
  ...
  const sessionToken = bootstrap.appSession && typeof bootstrap.appSession.token === "string" ? bootstrap.appSession.token : null;
  ...
  function getRuntimeHooks() {
    return isRecord2(kodyWindow.__kodyGeneratedUiRuntimeHooks) ? kodyWindow.__kodyGeneratedUiRuntimeHooks : {};
  }
  function getSessionRequestTarget2(type) {
    if (!sessionToken || !sessionEndpoints) {
      return null;
    }
    const url = type === "execute" ? sessionEndpoints.execute : type === "secrets" ? sessionEndpoints.secrets : sessionEndpoints.deleteSecret;
  ...
  async function listSecretsWithHttp(scope) {
    ...
    return payload.secrets.map((secret) => coerceSecretMetadata(secret)).filter((secret) => secret != null);
  }
  ...
  const kodyWidget2 = {
    ...
    async saveSecrets(input) {
      ...
      for (const item of input) {
        ...
        const response = await kodyWidget2.saveSecret(item);
        results.push({
          name: typeof item.name === "string" ? item.name : "",
          ok: response.ok === true,
          ...response.ok === true && response.secret ? { secret: response.secret } : {},
    ...
    async saveValue(input) {
      ...
      try {
        const result = await kodyWidget2.executeCode(
    ...
    async saveValues(input) {
      ...
      for (const item of input) {
        ...
        const response = await kodyWidget2.saveValue(item);
        results.push({
          name: typeof item.name === "string" ? item.name : "",
          ok: response.ok === true,
          ...response.ok === true && response.value ? { value: response.value } : {},
    ...
    async listValues(input) {
      const scope = coerceStorageScope(
        isRecord2(input) ? input.scope : void 0
      );
      const result = await kodyWidget2.executeCode(
      ...
      return result.values.map((value) => coerceValueMetadata(value)).filter((value) => value != null);
    },
    ...
    fillFromSearchParams(form, mapping) {
      ...
      const fieldNames = new Set(
        Array.from(resolvedForm.elements).map((element) => "name" in element ? element.name : "").filter((name) => typeof name === "string" && name.length > 0)
      );
      for (const name of fieldNames) {
        const fieldName = String(name);
        const mappedValue = mappingRecord?.[fieldName];
        const paramName = typeof mappedValue === "string" && mappedValue.length > 0 ? mappedValue : fieldName;
        const values = url.searchParams.getAll(paramName);
        if (values.length === 0) continue;
        setControlValues(resolvedForm, fieldName, values);
      }
      return kodyWidget2.formToObject(resolvedForm);
    },
    persistForm(form, options) {
      ...
      if (!isRecord2(options) || typeof options.storageKey !== "string" || options.storageKey.length === 0) {
        throw new Error("persistForm requires a storageKey option.");
      }
      const values = kodyWidget2.formToObject(resolvedForm);
      const fieldNames = Array.isArray(options.fields) ? options.fields.filter(
      ...
      for (const name of fieldNames) {
        ...
        const normalized = Array.isArray(value) ? value.filter((entry) => typeof entry === "string").map((entry) => entry) : typeof value === "string" ? value : null;
        if (normalized != null) {
    ...
    createOAuthState(key) {
      ...
      const state = globalThis.crypto && typeof globalThis.crypto.randomUUID === "function" ? globalThis.crypto.randomUUID() : Math.random().toString(36).slice(2) + Math.random().toString(36).slice(2);
    ...
    validateOAuthCallbackState(input) {
      if (!isRecord2(input) || typeof input.key !== "string" || input.key.length === 0) {
        throw new Error("validateOAuthCallbackState requires a key.");
      }
      const storage = getOAuthStorage();
      const expectedState = storage.getItem(input.key);
      const returnedState = typeof input.returnedState === "string" && input.returnedState.length > 0 ? input.returnedState : null;
      return {
        valid: typeof expectedState === "string" && expectedState.length > 0 && returnedState != null && expectedState === returnedState,
        expectedState,
        returnedState
      };
    },
    readOAuthCallback(input) {
      ...
      const expectedState = isRecord2(input) && typeof input.expectedStateKey === "string" ? storage.getItem(input.expectedStateKey) : null;
      return {
        kind: "success",
        code,
        state,
        callbackUrl: url.toString(),
        expectedState,
        stateMatches: expectedState != null && state != null ? expectedState === state : null
      };
    },
    ...
    async exchangePkceOAuthCode(input) {
      ...
      if (typeof input.tokenUrl !== "string" || typeof input.code !== "string" || typeof input.redirectUri !== "string" || typeof input.clientId !== "string" || typeof input.codeVerifier !== "string" || input.tokenUrl.length === 0 || input.code.length === 0 || input.redirectUri.length === 0 || input.clientId.length === 0 || input.codeVerifier.length === 0) {
        return {
          ok: false,
          kind: "execution_error",
          message: "exchangePkceOAuthCode requires tokenUrl, code, redirectUri, clientId, and codeVerifier."
        };
      }
      ...
      try {
        ...
        return {
          ok: false,
          kind: "execution_error",
          message: error instanceof Error ? error.message : "PKCE token exchange failed."
        };
      }
    },
    async exchangeOAuthCodeWithSecrets(input) {
      ...
      if (typeof input.tokenUrl !== "string" || typeof input.code !== "string" || typeof input.redirectUri !== "string" || typeof input.clientId !== "string" || typeof input.clientSecretSecretName !== "string" || input.tokenUrl.length === 0 || input.code.length === 0 || input.redirectUri.length === 0 || input.clientId.length === 0 || input.clientSecretSecretName.length === 0) {
        return {
          ok: false,
          kind: "execution_error",
          message: "exchangeOAuthCodeWithSecrets requires tokenUrl, code, redirectUri, clientId, and clientSecretSecretName."
        };
      }
      const scope = coerceStorageScope(input.scope);
      ...
      params.set("redirect_uri", input.redirectUri);
      appendOAuthExtraParams(params, input.extraParams);
      return await kodyWidget2.fetchWithSecrets({
        url: input.tokenUrl,
        method: "POST",
        headers: {
          Accept: "application/json",
    ...
    async saveOAuthTokens(input) {
      ...
      const secrets = [
        {
          ...
          value: accessToken,
          description: typeof input.accessTokenDescription === "string" ? input.accessTokenDescription : "OAuth access token",
      ...
      if (refreshToken && typeof input.refreshTokenSecretName === "string" && input.refreshTokenSecretName.length > 0) {
        secrets.push({
          ...
          value: refreshToken,
          description: typeof input.refreshTokenDescription === "string" ? input.refreshTokenDescription : "OAuth refresh token",
          scope: coerceStorageScope(input.scope) ?? void 0
        });
      }
      const saved = await kodyWidget2.saveSecrets(secrets);
      return {
        ok: saved.ok,
        accessTokenSaved: saved.results.some(
          (result) => result.name === input.accessTokenSecretName && result.ok === true
        ),
        refreshTokenSaved: typeof input.refreshTokenSecretName === "string" && input.refreshTokenSecretName.length > 0 ? saved.results.some(
          (result) => result.name === input.refreshTokenSecretName && result.ok === true
        ) : false,
        error: saved.ok ? void 0 : saved.results.find((result) => result.ok !== true)?.error || "Unable to save OAuth tokens.",
    ...
    buildSecretForm(input) {
      ...
      const controller = {
        form,
        save: async () => {
          const values = kodyWidget2.formToObject(form);
          ...
          const result = await kodyWidget2.saveSecrets(secrets);
          if (result.ok) {
...
function isNonNavigableUrl(value) {
  ...
  return normalizedValue === "" || normalizedValue.startsWith("#") || normalizedValue.startsWith("//") || normalizedValue.startsWith("about:") || normalizedValue.startsWith("blob:") || normalizedValue.startsWith("data:") || normalizedValue.startsWith("javascript:") || normalizedValue.startsWith("mailto:") || normalizedValue.startsWith("tel:");
}
...
function absolutizeSrcset(value, baseHref) {
  return value.split(",").map((candidate) => {
    ...
    return [absolutizeUrl(url, baseHref), ...descriptorParts].filter((part) => part.length > 0).join(" ");
  }).join(", ");
}
...
function absolutizeHtmlAttributeUrls(code, baseHref) {
  ...
  return code.replace(/<[^>]+>/g, (tag) => {
    ...
    return tag.replace(
      ...
      (match, prefix, attributeName, quotedValue, doubleQuotedValue, singleQuotedValue) => {
        ...
        const nextValue = attributeName.toLowerCase() === "srcset" ? absolutizeSrcset(decodedValue, baseHref) : absolutizeUrl(decodedValue, baseHref);
...
function renderGeneratedUiDocument(input) {
  ...
  const htmlSource = /<(?:!doctype|html|head|body)\b/i.test(input.code) ? injectIntoHtmlDocument(input.code, input.headInjection) : buildHtmlDocumentFromFragment(input.code, input.headInjection);
...
function coerceAppSession(value) {
  ...
  if (typeof value.sessionId !== "string" || typeof value.expiresAt !== "string") {
    return null;
  }
...
function coerceGeneratedUiEndpoints(value) {
  ...
  try {
    ...
    if (execute.origin !== origin || secrets.origin !== origin || deleteSecret.origin !== origin) {
      return null;
    }
    if (!source.pathname.startsWith("/ui-api/") || !execute.pathname.startsWith("/ui-api/") || !secrets.pathname.startsWith("/ui-api/") || !deleteSecret.pathname.startsWith("/ui-api/")) {
      return null;
    }
    return {
...
function getHostToolErrorMessage(result) {
  ...
  const structuredContent = isRecord3(result.structuredContent) ? result.structuredContent : null;
  const error = isRecord3(structuredContent?.error) ? structuredContent.error : null;
  return typeof error?.message === "string" ? error.message : "Code execution failed.";
}
function readSavedAppSourceFromHostToolResult(result) {
  ...
  const structuredContent = isRecord3(result.structuredContent) ? result.structuredContent : null;
  const code = typeof structuredContent?.code === "string" ? structuredContent.code : null;
  if (!code) {
    return {
      handled: true,
...
function coerceRenderEnvelope(value) {
  ...
  if (renderSource !== "inline_code" && renderSource !== "saved_app")
    return null;
  const code = typeof value.sourceCode === "string" ? value.sourceCode : typeof value.code === "string" ? value.code : void 0;
...
function getEnvelopeFromRenderData(renderData) {
  const toolOutput = isRecord3(renderData?.toolOutput) ? renderData.toolOutput : void 0;
...
function readGeneratedUiBootstrap() {
  ...
  return {
    mode: bootstrap.mode === "entry" || bootstrap.mode === "hosted" || bootstrap.mode === "mcp" ? bootstrap.mode : "entry",
...
function getSessionRequestTarget(appSession, type) {
  ...
  const url = type === "execute" ? appSession.endpoints.execute : type === "secrets" ? appSession.endpoints.secrets : appSession.endpoints.deleteSecret;
  return {
    url,
    token: appSession.token
  };
...
function buildSavedUiEndpoint(baseHref, uiId, endpoint) {
  ...
  const path = endpoint === "delete-secret" ? `/ui-api/${encodeURIComponent(uiId)}/secrets/delete` : `/ui-api/${encodeURIComponent(uiId)}/${endpoint}`;
...
async function observeRenderedDocumentSize(hostBridge) {
  ...
  const notifyMeasuredSize = async () => {
    ...
    if (!nextSize || lastMeasuredSize?.height === nextSize.height && lastMeasuredSize?.width === nextSize.width) {
      return;
    }
...
async function initializeRenderedMcpDocument(bootstrap) {
  ...
  const requestDisplayMode = async (mode) => {
    ...
    const nextMode = mode === "fullscreen" && displayMode === "fullscreen" ? "inline" : mode;
...
async function initializeShellHostDocument() {
  ...
  const resolveSavedAppCode = async (appId, appSession) => {
    ...
    if (!target) {
      throw new Error(
        hostToolResult.handled ? hostToolResult.errorMessage : "Failed to load saved app source."
      );
    }
    try {
      const targetToken = "token" in target && typeof target.token === "string" ? target.token : void 0;
      const { response, payload } = await fetchJsonResponse({
        url: target.url,
        method: "GET",
        token: targetToken
  ...
  const renderEnvelope = async (envelope) => {
    ...
    const buildDocument = (code, runtime) => renderGeneratedUiDocument({
      code,
      runtime,
    ...
    try {
      ...
      if (latestEnvelope !== envelope) return;
      const message = error instanceof Error ? error.message : "Unknown app loading error.";
