...
test('getRemoteAiLocalDevStartupError lists all missing remote AI startup env vars', () => {
	expect(
		...
		'AI_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID and CLOUDFLARE_API_TOKEN are required when AI_MODE is "remote" in local dev. Add them to packages/worker/.env before starting `npm run dev`.',
	)
})
