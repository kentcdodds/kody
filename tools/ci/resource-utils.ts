...
function addPackageAppCustomDomainRoute(input: {
	...
	// The package-app host itself is attached to the runtime Worker (ADR
	// 0016), not this one — its custom-domain route is generated by
	// tools/ci/runtime-worker-config.ts. The main Worker publishes the app
	// origin and any legacy origins.
	input.targetEnv.routes = [
		...existingRoutes,
		...[appHostname, ...legacyHostnames]
	...
	const legacyNote = legacyHostnames.length
		...
		: ''
	console.error(
		`Custom domain routes: ${appHostname} (APP_BASE_URL)${legacyNote}; workers.dev trigger kept. Package-app host ${packageAppHostname} (PACKAGE_APP_BASE_URL) is attached to the runtime worker.`,
	)
}
