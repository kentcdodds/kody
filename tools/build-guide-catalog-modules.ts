...
export async function ensureGuideCatalogModules() {
	...
	await writeAgentLog(
		'A',
		'tools/build-guide-catalog-modules.ts:ensure-entry',
		'guide generator entered',
		{ pid: process.pid },
	)
	// #endregion
	const filePaths = await findGuideMarkdownFiles()
	const stampContent = await buildStampContent(filePaths)
	const existingStamp = await readStamp()
	// #region agent log
	await writeAgentLog(
		'A',
		'tools/build-guide-catalog-modules.ts:stamp-check',
		'guide generator stamp checked',
		{
			pid: process.pid,
			fileCount: filePaths.length,
			stampMatched: existingStamp === stampContent,
		},
	)
	// #endregion
	if (existingStamp === stampContent) return
	...
	await writeAgentLog(
		'AB',
		'tools/build-guide-catalog-modules.ts:before-writes',
		'writing generated guide modules',
		{
			pid: process.pid,
	...
	await writeAgentLog(
		'AB',
		'tools/build-guide-catalog-modules.ts:after-fsync',
		'generated guide modules flushed',
		{ pid: process.pid, durationMs: Date.now() - startedAt },
	)
	// #endregion
}
