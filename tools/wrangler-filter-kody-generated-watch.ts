...
const watchBlockPattern =
	/if \(props\.findAdditionalModules\) \{\s*watchFiles = foundModulePaths(?:\.filter\(\(p\) => !p\.includes\("\.kody-generated"\)\))?;\s*const root = path\d+__namespace\.default\.resolve\(props\.entry\.moduleRoot\);\s*for await \(const dir\d+ of findAdditionalModuleWatchDirs\(root\)\) \{\s*watchDirs\.push\(dir\d+\);\s*\}\s*\}/
