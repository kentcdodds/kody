...
- `secret_buckets`: encrypted-secret ownership buckets scoped to `user`,
  ...
  auto-granted to self-authored packages (no `community_forks` row for that
  `saved_packages.id` + `userId`); packages forked from community listings
  (`community_forks.forked_package_id`, indexed in
  `0073-community-forks-forked-package-index.sql`) still require an explicit
  `allowed_packages` grant on every package read path.
...
- `secret_entries.allowed_hosts`, `secret_entries.allowed_capabilities`, and
  ...
  `allowed_packages` applies only to user-scoped secrets used by
  community-forked packages (provenance via `community_forks.forked_package_id`
  - `forker_user_id`; index `0073-community-forks-forked-package-index.sql`).
    Self-authored packages skip that grant. Package-scoped secrets are owned
