...
- `OAUTH_KV` supports OAuth client and token flows without custom storage code
  ...
  `OAuthHelpers` through the library's `getOAuthApi` (lazily imported, with the
  provider options shared from `packages/worker/src/oauth-provider-options.ts`)
  so grant, token, and client deletes hit the same keys.
