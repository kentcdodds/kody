...
rows stay inventoried until that lane clears the matching StorageRunner
Durable Objects and then deletes the rows, so account export, deletion, and
DR keep a user→storage_id map. Estimate reads treat those leftover rows as
StorageRunner (`unknown`). Index-backed repo-session
