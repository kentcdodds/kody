...
- Unverified person accounts: password signups that stay unverified
  ...
  fence for retry. Every outcome is durable: a purge writes an
  `unverified_account_purged` audit row and a failed deletion writes an
  `unverified_account_purge_failed` audit row (reason
  `<ErrorClassName>: <first inventory/cleanup warning or message>`, at most 200
  characters) plus a Sentry event, because Workers Logs sampling drops the
  `console.warn`. The admin-only `adminUnverifiedAccountPurgeRun` capability
  runs one bounded pass on demand (or previews the claim page with `dryRun`) and
  returns per-account outcomes keyed by stable user id. Social-login accounts
  are verified at creation and are not in this set.
