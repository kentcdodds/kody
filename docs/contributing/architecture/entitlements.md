...
- **Row-count limits** (saved packages, scheduled jobs, repo sessions, secrets,
  ...
  (RunLog-only authority; D1 `workflow_runs` retired after production parity
  2026-08-04): create reserves atomically via `reserveWorkflowProjectionSlot`,
  and usage readers call `countActiveWorkflowProjections` through
  `readCurrentEntitlementResourceUsage`. Terminal projections age-prune after 90
  days inside the DO; active / running rows are never age-pruned.
