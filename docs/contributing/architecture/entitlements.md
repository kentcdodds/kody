...
| `email_message_bytes`      | `handleInboundEmail` before quota/parse (per-message raw size via `resolvePlanLimit`). The MIME reader (`maxRawMimeBytes`) uses the same platform ceiling as `maxPlanEmailLimits.email_message_bytes`; oversize there rejects at SMTP instead of retrying.                                                                         |
