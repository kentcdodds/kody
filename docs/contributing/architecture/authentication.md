...
- Client registration: `/oauth/register` (via provider), plus Client ID Metadata
  ...
  by inbound OAuth `clientId`, 5-minute isolate and Durable Object debounce,
  `waitUntil` so it is not on the awaited hot path). Unknown last-used renders
  as "never" and is the revoke signal; Connected remains grant `createdAt`.
  Revoke deletes the last-used row with the grant. Onboarding Step 3 completion
  is unique `clientId`s ≥ 2, not raw grant count and not
