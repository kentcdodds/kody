...
Additional notification destinations (addresses `emailSend` may use besides
...
resend, set-default, and remove live at `/account` and
`/account/email-destinations.json` (3 requests per 15 minutes for add/resend).
The cap is 5 extras besides the identity email. Unverified extras never receive
mail.
