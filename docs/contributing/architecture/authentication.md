...
- Client registration: `/oauth/register` (via provider), plus Client ID Metadata
  ...
  `client_id` and skip registration entirely. MCP `2026-07-28` deprecates RFC
  7591 dynamic registration in favor of CIMD, so both stay enabled and clients
  that fail a CIMD fetch fall back to `/oauth/register`. CIMD metadata fetches
