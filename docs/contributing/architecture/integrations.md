...
immutable caching; projections expose the relative `logoPath`. The connect
page and account integration views render it, falling back to the built-in
`ProviderIcon` set.
...
(a connection named `apps` resolves at `/account/integrations/apps`). The
default connection and app views lead with the provider mark and the action
that matters (reconnect, or the accounts that share an app). Endpoints, secret
names, host allowlists, flow / PKCE / exchange style, and credential rotation
stay behind an advanced disclosure. The rotate form
