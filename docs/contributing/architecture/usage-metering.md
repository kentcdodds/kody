...
| `package_export`   | one saved-package bundled-code run              | `packages/worker/src/mcp/run-capabilities-registry.ts` (bundled runs with a package context) | package id                  |
...
| `outbound_fetch`   | one outbound fetch through the gateway          | `packages/worker/src/mcp/fetch-gateway.ts` (`CapabilitiesFetchGateway.fetch`)                | request host                |
