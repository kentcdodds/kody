...
| `durable_object_gb_seconds` | one typed per-user Durable Object RPC (wall-clock in `durationMs`; admin converts to GB-s at 128 MB) | `createMeteredDurableObjectStub` on `storageRunnerRpc` when `USAGE_EVENTS` is bound. Other per-user RPC factories can adopt the same helper; UserMeter, Mailbox, RunLog, and RepoSessionIndex stay unwrapped so admin usage reads do not inflate the metric. | DO class name                  |
