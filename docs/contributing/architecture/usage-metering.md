...
1. **Workers Analytics Engine is the write path in production and preview**
   ...
   - `doubles`: `[durationMs ?? 0, cpuMs ?? 0, bytes ?? 0]`. Coalesced
     `durable_object_gb_seconds` points store the RPC count in the third double
     instead of bytes so hourly rollups can recover `event_count`.
