...
| `dynamic_worker_invoke`     | one LOADER evaluate (hit or miss) on the execute-sandbox path                                                                                                                                                                   | `packages/worker/src/mcp/executor.ts` after `claimDynamicWorkerDay`, on every signed-in sandbox surface (execute, job, package_export, workflow, …). Observe-only. `cacheReuse` is `miss` when `claimDynamicWorkerDay.created === true` and `hit` otherwise. Also carries `codeChars`, `durationMs`, `surface`, `executeShape` when known, and `hadParams`. No worker id, source, params, or package names.         | none                           |
...
summed duration). Hit rate and `hadParams` share are Analytics Engine queries.
Use `buildDynamicWorkerInvokeReuseQuery` and `buildDynamicWorkerReuseRatioQuery`
in `packages/worker/src/usage/aggregate-rollups.ts`:
...
1. **Workers Analytics Engine is the write path in production and preview**
   ...
   - `indexes`: `[userId]`
   - `blobs`:
     `[userId, eventType, entityId ?? '', outcome, timestamp, surface ?? '', executeShape ?? '', cacheReuse ?? '', hadParams]`
     (`surface` is blob6, `executeShape` is blob7, `cacheReuse` is blob8,
     `hadParams` is blob9 as `true` / `false`; all empty when unset)
   - `doubles`: `[durationMs ?? 0, cpuMs ?? 0, bytes ?? 0, codeChars ?? 0]`.
     ...
     (module-graph text length on `dynamic_worker_invoke`).
