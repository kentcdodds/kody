...
**Inventory/retention detach deploy (code-only, PR #1206):** removes legacy D1
projection tables from account deletion/export inventory and retires the hourly
D1 `workflow_runs` retention lane. Physical D1 tables (`workflow_runs`,
`user_package_run_successes`, `user_activation_milestones`) remain as quiescent
rollback copies pending a migration-only follow-up.

**Migration-only follow-up deploy (separate PR, migration `0137`):** drops the
quiescent D1 projection tables after deploy 2 is live. Pre-drop D1 Time Travel
bookmark for database `8c1014d1-6b41-4695-a0a2-159071f0f919`:
...
and `user_package_run_successes` are quiescent rollback copies outside account
inventory and are not queried on that path.
