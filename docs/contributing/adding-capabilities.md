...
registration in `search-entity-registry.ts`, and parallel updates to the closed
unions in `search-format-types.ts` plus detail routing in `search-detail.ts`
(and `parseEntityRef` when the type is entity-backed).
