...
commit key so the identity row never points at a hole. Shared modules, missing
prior artifacts, first publish, force, and mismatched `already_published`
snapshot rewrites still rebuild. Check failures before promotion return the
failed checks and do not mutate D1, KV snapshots, published bundle artifacts,
package projections, or vectors. A rebuild failure after promotion returns
