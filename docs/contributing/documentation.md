...
`========` / `----------` comment banners. `npm run tautological-absence:check`
rejects vanished-copy `not.toContain` leftovers in tests. `npm run knip` fails
on unused files against the configured entrypoints. These run as part of
`npm run validate`.
