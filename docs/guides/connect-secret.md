...
\`\`\` https://heykody.dev/connect/secret? name=linearApiKey &description=Linear
API key for issue management &allowedHosts=api.linear.app &scope=user
&dashboardUrl=https://linear.app/settings/api &instructions=Go to Linear
Settings → API → Personal API Keys → Create key
&allowedCapabilities=linear_issue_list,linear_issue_create &connector=linear
\`\`\`
