...
code is written, or `import(specifier)` when the name is data. Package
invocation tokens are for external systems such as webhooks, gateway proxies,
CLIs, or other trusted personal clients that call Kody over HTTP.
...
orchestrator or discovery package) and statically import it inside Kody, or they
speak MCP.
