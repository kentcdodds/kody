...
1. Call `packageGetGitRemote`. For a new package, pass `create: true`, leftover
   `kody_id` set to the `@owner/leaf` name (or the name leaf), and `description`
   (at most 200 characters; a short public tagline); for an existing package,
   omit `create` and pass leftover `kody_id` or `package_id` when the name is
   not known.
