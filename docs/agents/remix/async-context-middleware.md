...
Source:
https://github.com/remix-run/remix/tree/main/packages/async-context-middleware

## README

Middleware for storing request context in `AsyncLocalStorage` for use with
[`@remix-run/fetch-router`](https://github.com/remix-run/remix/tree/main/packages/fetch-router).

This middleware stores the request context in
[`AsyncLocalStorage`](https://nodejs.org/api/async_context.html#class-asynclocalstorage)
(using `node:async_hooks`), making it available to all functions in the same
...
Simply use the `asyncContext()` middleware at the router level to make the
request context available to all functions in the same async execution context.
Get access to the context using the `getContext()` function.
