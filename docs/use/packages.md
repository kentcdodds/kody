...
Individual files may be at most 10 MiB. Publish checks reject any file over
that per-file limit (and any source root over the aggregate publish caps) with
...
remote itself also fails pushes above roughly 32 MiB of pack content with a raw
HTTP 413 before Kody is involved, so oversized files can fail at `git push`
with an unhelpful error even before publish checks run.
