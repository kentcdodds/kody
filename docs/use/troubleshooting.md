...
tool as a model default or pre-enabled tool — enable it in the chat so the
host can open the authorize window. Hosts that require a pre-registered
Client ID and Client Secret can mint one at Account → Advanced → MCP OAuth
clients and use the host’s static OAuth mode. Static mode still runs authorize
and token exchange.
