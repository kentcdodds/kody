...
- Admin-only delivery rules (for example `platform.feedback.submitted`,
  `status.incident.opened`, `fleet.package_error_rate.elevated`, `user.created`,
  or `user.deleted`) apply to **production** fan-out only; synthetic dispatch
  runs your handler directly for smoke testing.
